...
class TestLlamaParseBackend:
    ...
    async def test_parse_with_cache_miss(self, tmp_path):
        ...
        backend.client.create_job_with_retry = AsyncMock(return_value="job123")
        backend.client.poll_for_result_with_retry = AsyncMock(return_value="markdown content")

        results = await backend.parse(["file.pdf"])

        assert results == [str(tmp_path / "new.md")]
        backend.client.poll_for_result_with_retry.assert_awaited_once()
    ...
    @pytest.mark.asyncio
    async def test_parse_with_api_error(self, capsys):
        backend = LlamaParseBackend(config_path="dummy", verbose=True)
        backend.cache_manager = MagicMock()
        backend.cache_manager.should_skip_file.return_value = False
        backend.cache_manager.get_cached_result = AsyncMock(return_value=None)
        backend.client = MagicMock()
        backend.client.create_job_with_retry = AsyncMock(side_effect=Exception("API Error"))
