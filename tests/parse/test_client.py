...
class TestParseClient:
    ...
    async def test_create_parse_job_success(self, llama_parse_config, tmp_path):
        ...
        mock_http_client = AsyncMock(post=AsyncMock(return_value=Response(200, json={"id": "job123"}, request=Request("POST", "http://dummy.url"))))
        file = tmp_path / "test.pdf"
        file.touch()

        job_id = await client.create_parse_job(mock_http_client, str(file))
    ...
    async def test_create_parse_job_http_error(self, llama_parse_config, tmp_path):
        client = ParseClient(llama_parse_config)
        mock_http_client = AsyncMock(post=AsyncMock(return_value=Response(500, text="Server Error", request=Request("POST", "http://dummy.url"))))
        file = tmp_path / "test.pdf"
        file.touch()

        with pytest.raises(ParseHttpError):
            await client.create_parse_job(mock_http_client, str(file))
    ...
    async def test_get_job_result_success(self, llama_parse_config):
        ...
        result = await client.get_job_result(mock_http_client, "job123")
    ...
    async def test_get_job_result_timeout(self, llama_parse_config, mocker):
        ...
        client = ParseClient(llama_parse_config)
        mock_http_client = AsyncMock()
        mock_http_client.get.return_value = Response(200, json={"status": JobStatus.PENDING}, request=Request("GET", "http://dummy.url"))
        with pytest.raises(ParseTimeoutError):
            await client.get_job_result(mock_http_client, "job123")

    @pytest.mark.asyncio
    async def test_get_job_result_job_failure(self, llama_parse_config):
        client = ParseClient(llama_parse_config)
        mock_http_client = AsyncMock()
        mock_http_client.get.return_value = Response(200, json={"status": JobStatus.ERROR}, request=Request("GET", "http://dummy.url"))
        with pytest.raises(ParseHttpError):
            await client.get_job_result(mock_http_client, "job123")
    ...
    async def test_create_job_with_retry(self, llama_parse_config, tmp_path):
        ...
        file = tmp_path / "test.pdf"
        file.touch()
        job_id = await client.create_job_with_retry(mock_http_client, str(file))
        assert job_id == "job123"
    ...
    async def test_poll_for_result_with_retry(self, llama_parse_config):
        ...
        result = await client.poll_for_result_with_retry(mock_http_client, "job123")
        assert result == "content"